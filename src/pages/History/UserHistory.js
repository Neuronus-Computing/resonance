...
import { fetchTransactionsByReceiverId } from "../../store/actions";
...
class UserHistory extends Component {
  ...
  fetchTransactions = async () => {
    const receiverId = this.props.router.location.pathname.split("/").pop()
    if (receiverId) {
      ...
      this.props.fetchTransactionsByReceiverId(
        adjustedStartDate,
        adjustedEndDate,
        receiverId,
        (wallets) => {
      ...
      toast.error("Invalid contact address.")
    }
  };
  handleCalendarClick = () => {
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <div className="page-freeze h-100">
          <Container className="w-size h-100">
            <div className="crypto-content">
              <h1 className="crypto-heading m-0">
                ...
                <span className="mx-2">Transactions History</span>
              </h1>
                          ...
                          <CardBody>
                            ...
                            >
                              <h5 className="mb-0">
                                {wallet.label}{" "}
                                <i
                                  className={`ri-arrow-${collapseStates[wallet.id] ? "up" : "down"
                                    }-s-line`}
                                ></i>
                              </h5>
                            </Button>
                                            ...
                                            <div className="detail-item">
                                              <div className="month-detial">
                                                ...
                                                <span className="id-detial">
                                                  ...
                                                  <p>
                                                    {moment(tx.timestamp).format("YYYY-MM-DD")} | {moment(tx.timestamp).format("HH:mm:ss")}
                                                  </p>
                                                </span>
                                              </div>
                                              <span
                                                className={`${tx.type === "send" ? "value" : "value-cls"
                                                  }`}
                                              >
                                                {tx.type === "send" ? "-" : "+"}
                                                ${tx.amount}
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(UserHistory));
